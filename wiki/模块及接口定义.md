...
- 输出信号说明
	|名称			|去向		|位宽|描述			|
	|:---:|:---:|:---:|:----|
	|inst_		|EXCEPTION.EXCEPTION_REG._inst/EX.EX_CU._inst	|32	|即将进入执行阶段的指令			|
	|pc_		|EXCEPTION.EXCEPTION_REG._pc		|32	|即将进入执行阶段的指令的地址	|
	|pc_sub4_	|EXCEPTION.EXCEPTION_REG._pc_sub4	|32	|即将进入的pc-4用于延迟槽指令的异常处理返回地址		|
	|pc_add4_	|EXCEPTION.EXCEPTION_REG._pc_add4/EX.ADD._pc_add4	|32	|即将进入的pc+4用于执行阶段计算b类型分支指令的跳转目标			|
	|pc_add8_	|EXCEPTION.EXCEPTION_REG._pc_add8	|32	|即将进入的pc+8用于链接跳转指令的地址保存				|
	|exc_		|EXCEPTION.EXCEPTION_REG._exc/EX.EX_EXC._exc		|32	|即将进入执行阶段的异常向量				|
	|data1_		|EX.ALU._data1					|32	|即将进入执行阶段的alu原操作数1				|
	|data2_		|EX.ALU._data2					|32	|即将进入执行阶段的alu原操作数2				|
	|imm32_		|EX.ADD._imm32			|32	|即将进入执行阶段的拓展之后的立即数			|
	|delaysolt_	|EXCEPTION.EXCEPTION_REG._delaysolt	|即将进入执行阶段的指令是否是延迟槽指令				|
- [CPU](#cpu)
